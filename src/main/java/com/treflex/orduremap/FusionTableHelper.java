...
public class FusionTableHelper {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger("Fusion helper");
	...
	public void insert(final Ordure ordure) throws IOException, ServiceException {
		service = new GoogleService("fusiontables", "orduremap");
		service.setUserCredentials("global.garbage.map@gmail.com", "o1r9d8u3re", ClientLoginAccountType.GOOGLE);
		LOGGER.info("Insertion de {} dans fusion tables", ordure);
		...
		writer.flush();
		request.execute();
		LOGGER.info("Insertion de {} a été terminé avec succès", ordure);
	}
}
